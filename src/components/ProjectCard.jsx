...
const ProjectCard = ({ project, onSelect, index }) => {
    ...
    return (
        <motion.div
            ...
            className="cursor-pointer bg-[var(--button-color)] hoverLight soft rounded-[12px] border border-transparent hover:border-[var(--accent-color)] p-[1.4rem] flex flex-col gap-6 w-[45rem] mx-auto min-h-[360px] max-[1480px]:w-[37rem] max-[1250px]:w-[30rem] max-[1100px]:w-[80%] max-sm:w-[90%]"
        >
            <img
                src={project.img}
                alt={project.title}
                className="rounded-md aspect-[16/10] object-cover w-full h-40"
            />
            <h2
...
const ProjectDetails = ({ project, onClose }) => {
  return (
    <motion.div
      layoutId={`card-${project.id}`}
      className="bg-[var(--button-color)] soft rounded-[40px] max-w-[45rem] mx-auto pt-0 p-[1.6rem] relative z-50 max-[800px]:max-w-[90%]"
    >
        <img
            src={project.img}
            alt={project.title}
            className="rounded-[40px] w-full aspect-[16/10] object-cover py-4"
        />
